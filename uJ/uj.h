...
#define UJ_ERR_NONE                                                            \
    0 //				not an error (may mean true)
#define UJ_ERR_FALSE                                                           \
    1 //				also not an error - just means false

#define UJ_ERR_INVALID_OPCODE 2 // InternalError [?]
#define UJ_ERR_METHOD_NONEXISTENT                                              \
    3 // UnknownError [?]		ujThreadGoto to an invalid place
#define UJ_ERR_DEPENDENCY_MISSING                                              \
    4 // NoClassDefFoundError		ujClassLoad fails because needed
...
#define UJ_ERR_ARRAY_INDEX_OOB                                                 \
    21 // IndexOutOfBoundsException	index out of bounds
#define UJ_ERR_FIELD_NOT_FOUND                                                 \
    22 // UnknownError [?]		such a field doe snot exist (by name)
#define UJ_ERR_NULL_POINTER 23 // NullPointerException
#define UJ_ERR_MON_STATE_ERR                                                   \
    24 // IllegalMonitorStateException	monitor state exception
#define UJ_ERR_NEG_ARR_SZ 25 // NegativeArraySizeException

#define UJ_ERR_RETRY_LATER                                                     \
    50 // 				not an error, just retry later

#define UJ_ERR_USER_EXCEPTION                                                  \
    99 // 				in case of exceptions disabled... or
       // uncaught
#define UJ_ERR_INTERNAL                                                        \
    100 // UnknownError [?]		bad internal error
