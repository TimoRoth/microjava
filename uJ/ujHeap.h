...
void *ujHeapHandleIsLocked(
    HANDLE handle); // return pointer if already locked, else NULL

void ujHeapUnmarkAll(void);
void ujHeapFreeUnmarked(void);
HANDLE
ujHeapFirstMarked(UInt8 markVal); // get first handle with a given mark value
void ujHeapMark(HANDLE handle,
                UInt8 mark); // will only increase the mark value
